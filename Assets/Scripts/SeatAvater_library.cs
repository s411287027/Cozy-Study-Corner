...
public class SeatAvatar_library : MonoBehaviour
{
    ...
    private DatabaseReference firebaseRef; // ⭐ 修正點 1: 儲存 Firebase 參考，方便取消監聽
    ...
    void Start()
    {
        ...
        Debug.Log("[SeatAvatar_library] 開始監聽 Firebase 數據...");

        // ⭐ 修正點 2: 設置固定的 Firebase 路徑 ("Seat/Library")
    ...
    private void OnSeatValueChanged(object sender, ValueChangedEventArgs e)
    {
        ...
        if (e.Snapshot == null)
        {
            Debug.LogWarning("[SeatAvatar_library] Firebase 快照為空，跳過更新。");
            return;
        }
