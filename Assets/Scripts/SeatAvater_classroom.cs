...
// 不需要 using UnityEngine.SceneManagement; 因為是固定路徑

// 假設檔案名是 SeatAvatar_classroom.cs，類別名稱必須匹配
public class SeatAvatar_classroom : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("[SeatAvatar_classroom] 開始監聽 Firebase 數據...");

        // ⭐ 修正點 2: 設置 Firebase 參考並儲存
    ...
    private void OnSeatValueChanged(object sender, ValueChangedEventArgs e)
    {
        ...
        if (e.Snapshot == null)
        {
            Debug.LogWarning("[SeatAvatar_classroom] Firebase 快照為空，跳過更新。");
            return;
        }
