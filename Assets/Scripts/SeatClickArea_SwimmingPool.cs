...
public class SeatClickArea_SwimmingPool : MonoBehaviour
{
    public string seatId;
    public SeatManager_Swimmingpool manager;
    ...
    {
        Debug.Log($"[SeatClickArea_SwimmingPool] 嘗試加好友，座位: {seatId}");
        HideButtons();

        if (manager == null)
        {
            Debug.LogError("❌ SeatClickArea_SwimmingPool 找不到 SeatManager_Swimmingpool 引用！");
            return;
        }

        string currentRoom = manager.currentRoomID;
        string path = $"Seat/Swimmingpool/{currentRoom}/{seatId}";
    ...
    {
        Debug.Log($"[SeatClickArea_SwimmingPool] 點擊便條紙功能: {seatId}");
        HideButtons();
        ...
        FirebaseDatabase.DefaultInstance.RootReference.Child(path)
            .GetValueAsync().ContinueWithOnMainThread(task =>
            {
                ...
                stickyNoteUI.OpenSendPanel(targetUid, "SwimmingPool");
            });
    }
