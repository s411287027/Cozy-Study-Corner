...
public class SeatAvatar_Cafe : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("[SeatAvatar_Cafe] 開始監聽 Firebase 數據...");

        // ⭐ 修正點 3: 設置固定的 Firebase 路徑 ("Seat/Coffee")
        // 注意：您的路徑是 "Seat/Coffee"，請確保 Firebase 資料結構中是 "Coffee" 節點。
    ...
    private void OnSeatValueChanged(object sender, ValueChangedEventArgs e)
    {
        ...
        if (e.Snapshot == null)
        {
            Debug.LogWarning("[SeatAvatar_Cafe] Firebase 快照為空，跳過更新。");
            return;
        }
