...
public class SeatAvatar_camp : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (avatarDB == null)
        {
            Debug.LogError("❌ 錯誤：請在 SeatAvatar_camp 元件中放入 AvatarDatabase 設定檔！");
            return;
        }
    ...
    public void ConnectToRoom(string roomID)
    {
        ...
        Debug.Log($"[SeatAvatar_Camp] 準備切換至房間：{currentRoomID}");

        // ⭐ 設定新的監聽路徑：Seat/Camp/RoomX
        firebaseRef = FirebaseDatabase.DefaultInstance.GetReference($"Seat/Camp/{currentRoomID}");
        firebaseRef.ValueChanged += OnSeatValueChanged;
    }
    ...
    private void ApplyAppearance(AppearanceTask task)
    {
        ...
        if (targetSeat.currentAvatarObj != null)
        {
            ...
            if (targetSeat.runtimeSleeve != null)
            {
                ...
                targetSeat.runtimeSleeve.sortingOrder = baseOrder + 22;
    ...
    private void ProcessSeatUpdates(DataSnapshot snapshot)
    {
        ...
        if (currentRoomID != lastDisplayedRoomID)
        {
            ...
            {
                Debug.Log($"[SeatAvatar_Camp] 收到 {currentRoomID} 資料，清除舊房間 ({lastDisplayedRoomID}) 顯示...");
