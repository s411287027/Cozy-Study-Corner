...
public class SeatAvatar_swimmingpool : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("[SeatAvatar_swimmingpool] 開始監聽 Firebase 數據...");

        // 設置 Firebase 參考並儲存
        // ⭐ 注意：這裡將 'Swimmingpoll' 修正為 'Swimmingpool' (如果您 Firebase 節點是這個拼寫)
    ...
    private void OnSeatValueChanged(object sender, ValueChangedEventArgs e)
    {
        ...
        if (e.Snapshot == null)
        {
            Debug.LogWarning("[SeatAvatar_swimmingpool] Firebase 快照為空，跳過更新。");
            return;
        }
