...
public class SeatClickArea_Camp : MonoBehaviour
{
    public string seatId;
    public SeatManager_Camp manager;
    ...
    {
        Debug.Log($"[SeatClickArea_Camp] 嘗試加好友，座位: {seatId}");
        HideButtons();

        if (manager == null)
        {
            Debug.LogError("❌ SeatClickArea_Camp 找不到 SeatManager_Camp 引用！");
            return;
        }

        string currentRoom = manager.currentRoomID;
        string path = $"Seat/Camp/{currentRoom}/{seatId}";
    ...
    {
        Debug.Log($"[SeatClickArea_Camp] 點擊便條紙功能: {seatId}");
        HideButtons();
        ...
        FirebaseDatabase.DefaultInstance.RootReference.Child(path)
            .GetValueAsync().ContinueWithOnMainThread(task =>
            {
                ...
                stickyNoteUI.OpenSendPanel(targetUid, "Camp");
            });
    }
