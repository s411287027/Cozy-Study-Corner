...
public class SeatManager_Swimmingpool : MonoBehaviour
{
    ...
    public void ConnectToRoom(string roomId)
    {
        ...
        Debug.Log($"[SeatManager_Swimmingpool] 切換操作目標至：{currentRoomID}");

        // 3. 設定新的監聽路徑：Seat/Swimmingpool/RoomX
        roomRef = FirebaseDatabase.DefaultInstance.GetReference($"Seat/Swimmingpool/{currentRoomID}");
        roomRef.ValueChanged += OnSeatDataChanged;
    }
    ...
    private void OnSitButtonClicked(string seatId)
    {
        ...
        // ⭐ 修正 5: 寫入路徑加入 Room ID
        string seatPath = $"Seat/Swimmingpool/{currentRoomID}/{seatId}";
    ...
    // 保存目前顯示按鈕的 ClickArea
    private SeatClickArea_SwimmingPool activeClickArea = null;

    // ⭐ 點擊座位顯示按鈕
    public void OnSeatClicked(string seatId, SeatClickArea_SwimmingPool clickArea)
    {
        string path = $"Seat/Swimmingpool/{currentRoomID}/{seatId}";
